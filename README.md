...
- ✅ 回归测试扩至 **31 文件 / 2124 项断言 / 0 失败**：本版新增**闪蒸 79 项**（饱和水/汽物性独立锚点与 ρ_g 复核 / 释放功率与表格口径逐列手算锚点 / 严格口径与「汽化焓填潜热」偏差 21.7 % / 汽量线性性 / 干物守恒与浓缩 / 二次蒸汽体积与管径 / 温度-压力硬约束 / **计算链 9 组联动断言**：登记项、取上游值填 5 栏、行标记、过期提示、手改解绑、clear 解引用、本页不作为自己的上游）；喷射液化器 **128 项**（v1.9.1 固化，本版为其出口状态接上「计算链登记」）
