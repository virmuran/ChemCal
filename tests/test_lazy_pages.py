...
**为什么有这个文件**：50 个计算器 + 21 个换算页原先在启动时**全部实例化**，
...
check("导航项数为 50（50 个计算器）", NAV == 50, NAV)

built = [i for i, p in enumerate(cc.pages) if not is_placeholder(p)]
check("构造后只有 1 个真页面（首行）", built == [0], built)
check("其余 49 页都是占位页", sum(1 for p in cc.pages if is_placeholder(p)) == 49)
