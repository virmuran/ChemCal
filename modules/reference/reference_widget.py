...
本次（2026-09-16）改造解决四件事：
  1. **分类维度混乱**：14 类混着「按专业/按资料类型/按物料」三套维度，
     ...
     `group`（顶层分组，统一按用途）与 `tags`（条目标签，可筛选），
    ...
    if sec is not None and not set(add_headers) & set(sec.get("headers") or []):
        ...
        for row in sec.get("rows", []):
            ...
            if pair:
                ...
                row.extend(["—", "—"])
        sec["note"] = ("前 4 列为 25°C 值（《化工工艺设计手册》/Perry's）；后 2 列为 20°C 值，"
                       "取自 reference_data.FLUID_PROPERTIES —— 该表**只供本表做对照**，"
                       "计算器各自带自己的流体数据，不读这一份。同一物质两处数值不同是"
                       "**温度基准差异**，不是矛盾 —— 例如水 997/0.89（25°C）与 998.2/1.002（20°C）。"
                       "标「—」者无公认的 20°C 单一值（载冷剂按浓度、导热油/柴油按牌号而变），"
                       "**宁缺不造数** —— 需要时请按实际浓度或牌号查专用表。")
