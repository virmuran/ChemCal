...
class GasMixturePropertiesCalculator(CalculatorBase):
    """气体混合物物性计算器 - 统一UI风格版"""
    ...
    def calculate_mixture_properties(self, components, T, P, mixture_type, method):
        ...
        if mixture_type == "理想气体":
            # P[kPa] × mw[g/mol] / (R[kPa·L/(mol·K)] × T[K]) = g/L = kg/m³
            density = P * mw_mix / (8.314 * T_k)
        else:
            density = P * mw_mix / (z_factor * 8.314 * T_k)
